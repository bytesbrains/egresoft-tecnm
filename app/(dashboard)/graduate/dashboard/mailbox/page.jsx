...
export default function Mailbox() {
  ...
  return (
    <Grid container spacing={3} sx={{pt:{
      md:'20px'
    }}}>
      <Grid item xs={12} sx={{
        display:{
      ...
      {currentTab === 'Inicio' ? (<>
      <Grid item xs={12} md={8} sx={{ display:'flex', alignItems:'center', justifyContent:'space-between', height: '40px' }}>
        <SortInput />
        <Box
          sx={{
            display: 'flex',
        ...
        {data.map((item) => (
          ...
          >
            ...
            >
              ...
              <Typography variant='h6' style={{ marginLeft: '8px', fontSize:'16px'}}>
                {item.title}
              </Typography>
         ...
         <Grid item md={4} sx={{width:'300px', display:{
          xs:'none',
          md:'block'
         }}} >
        <Link href='https://www.occ.com.mx/'>
          <img style={{width:'100%'}} src='/images/InfografiaUniversidades.jpg' alt='bolsa de trabajo'/>
        </Link>
        <Link href='https://www.occ.com.mx/'>
          <img style={{width:'100%'}} src='/images/sne-bolsa-de-trabajo.jpg' alt='bolsa de trabajo'/>
        </Link>
      </Grid></>)
      : (
